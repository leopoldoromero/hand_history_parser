...
def test_upload_valid_history_file(sample_hand_history):
    ...
    response = client.post("/api/v1/history", files=files)

    assert response.status_code == 200, f"Unexpected status code: {response.status_code}"
    assert "data" in response.json(), "Response does not contain 'data' key"
    assert isinstance(response.json()["data"], list), "Expected 'data' to be a list"
...
def test_upload_malformed_hand_history():
    ...
    assert response.status_code in [200, 500]  # Adjust based on expected handling of bad input
